...
public class ActionAnimation extends MainFrameAnimation {
	public static final TransformSheet EMPTY_SHEET = new TransformSheet(
	...
	public void begin(LivingEntityPatch<?> entitypatch) {
		...
		if (entitypatch.shouldMoveOnCurrentSide(this)) {
			 ...
			 * entitypatch.getOriginal().setDeltaMovement(0.0D,
			 * entitypatch.getOriginal().getDeltaMovement().y, 0.0D);
			...
			MoveCoordSetter moveCoordSetter = this.getProperty(ActionAnimationProperty.COORD_SET_BEGIN)
	...
	protected void move(LivingEntityPatch<?> entitypatch, DynamicAnimation animation) {
		...
		if (this.getState(EntityState.INACTION, entitypatch,
				entitypatch.getAnimator().getPlayerFor(this).getElapsedTime())) {
	...
	public void modifyPose(DynamicAnimation animation, Pose pose, LivingEntityPatch<?> entitypatch, float time,
		...
		OpenMatrix4f toRootTransformApplied = entitypatch.getArmature().searchJointByName("Root").getLocalTrasnform()
				.removeTranslation();
		...
		worldPosition.y = (this.getProperty(ActionAnimationProperty.MOVE_VERTICAL).orElse(false)
				&& worldPosition.y > 0.0F) ? 0.0F : worldPosition.y;
	...
	@Override
	public void setLinkAnimation(DynamicAnimation fromAnimation, Pose startPose, boolean isOnSameLayer,
		...
		PlaybackSpeedModifier playSpeedModifier = this.getRealAnimation()
		...
		if (entitypatch.shouldMoveOnCurrentSide(this)
				&& this.getProperty(ActionAnimationProperty.MOVE_ON_LINK).orElse(true)) {
	...
	public void removeRootTranslation(LivingEntityPatch<?> entitypatch, Pose pose, float poseTime) {
		...
		if (this.getProperty(ActionAnimationProperty.COORD).isEmpty()) {
			Vec3f withPosition = entitypatch.getArmature().getActionAnimationCoord()
					.getInterpolatedTranslation(poseTime);
	...
	protected Vec3 getCoordVector(LivingEntityPatch<?> entitypatch, DynamicAnimation animation) {
		...
		MoveCoordSetter moveCoordsetter = isCoordUpdateTime
				? this.getProperty(ActionAnimationProperty.COORD_SET_TICK).orElse(null)
		...
		boolean moveVertical = this.getProperty(ActionAnimationProperty.MOVE_VERTICAL)
				.orElse(this.getProperty(ActionAnimationProperty.COORD).isPresent());
		MoveCoordGetter moveGetter = isCoordUpdateTime
				? this.getProperty(ActionAnimationProperty.COORD_GET).orElse(MoveCoordFunctions.DIFF_FROM_PREV_COORD)
				: MoveCoordFunctions.DIFF_FROM_PREV_COORD;
		...
		this.getProperty(ActionAnimationProperty.NO_GRAVITY_TIME).ifPresentOrElse((noGravityTime) -> {
			...
			if (moveVertical && move.y > 0.0F && !hasNoGravity) {
				...
				livingentity.setDeltaMovement(motion.x, motion.y <= 0.0F ? (motion.y +
						gravity) : motion.y, motion.z);
